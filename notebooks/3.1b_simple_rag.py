...
# MAGIC
# MAGIC First, let's create a function to retrieve relevant
# MAGIC documents from our vector search index.
...
docs = retrieve_documents(query, num_results=5)
...
""")
    ...
    prompt = (
        ...
        " information, say so"
        "\n- Cite the relevant paper titles when"
        " making claims"
...
result = rag_query("What are the best practices for efficient use of tokens in VLMs?")
...
result2 = rag_query("How do visual language models handle token limits?")
...
class SimpleRAG:
    ...
    def chat(self, question: str, num_docs: int = 3) -> str:
        ...
        system_message = (
            "You are a helpful research assistant."
            " Use the following context from research"
            " papers to answer questions."
            f"\n\nCONTEXT:\n{context}"
            "\n\nIf the context doesn't contain relevant"
            " information, say so. Always cite paper"
            " titles when making claims."
        )
...
# First question
q1 = "How Visual Language Models handle token limits?"
a1 = rag.chat(q1)
logger.info(f"Q: {q1}")
logger.info(f"A: {a1}\n")
...
q2 = "How can we optimize token usage in Visual Language Models?"
a2 = rag.chat(q2)
logger.info(f"Q: {q2}")
logger.info(f"A: {a2}\n")
...
# Another follow-up
q3 = "What are the computational costs in Visual Language Models?"
a3 = rag.chat(q3)
logger.info(f"Q: {q3}")
logger.info(f"A: {a3}")
