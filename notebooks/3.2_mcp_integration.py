...
# MAGIC
# MAGIC **MCP** is a standardized protocol for connecting
# MAGIC AI models to external data sources and tools.
# MAGIC
# MAGIC ### Key Concepts:
# MAGIC
...
search_result = vs_mcp_client.call_tool(
    ...
    {
        "query": (
            "What are best practices to reduce token usage in visual language models?"
        ),
    },
)
...
if vector_search_tool_name in tools_dict:
    ...
    result = search_tool.exec_fn(
        query="what Video Language Models are more token efficient?"
    )
...
response = agent.chat("Find papers about Video Language Models.")
